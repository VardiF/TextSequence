...
def test_clip_mutation_endpoints_return_authoritative_state(tmp_path):
    ...
    split = client.post(f"/api/projects/{project.id}/clips/split", json={"clip_id": project.tracks[0].clips[0].id, "timeline_frame": 40, "expected_revision": 0})
    assert split.status_code == 200
    split_project = split.json()
    assert split_project["revision"] == 1
    assert len(split_project["tracks"][0]["clips"]) == 2
    second_id = split_project["tracks"][0]["clips"][1]["id"]

    conflict = client.post(f"/api/projects/{project.id}/clips/move", json={"clip_id": second_id, "timeline_start_frame": 0, "expected_revision": 1})
    assert conflict.status_code == 400
    stale = client.post(f"/api/projects/{project.id}/clips/delete", json={"clip_id": second_id, "expected_revision": 0})
    assert stale.status_code == 409

    moved = client.post(f"/api/projects/{project.id}/clips/move", json={"clip_id": second_id, "timeline_start_frame": 120, "expected_revision": 1})
    assert moved.status_code == 200
    assert moved.json()["revision"] == 2

    trimmed = client.post(f"/api/projects/{project.id}/clips/trim", json={"clip_id": second_id, "source_out_frame": 80, "expected_revision": 2})
    assert trimmed.status_code == 200
    assert trimmed.json()["revision"] == 3
