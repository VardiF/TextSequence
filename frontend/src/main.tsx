...
type Asset = { id: string; kind: 'video'; path: string; name: string; codec: string; width: number; height: number; fps: FrameRate; duration_frames: number };
type Clip = { id: string; kind: 'video'; track_id?: string; asset_id: string; source_in_frame: number; source_out_frame: number; timeline_start_frame: number };
type Track = { id: string; name: string; kind: 'video'; external_refs: unknown[]; clips: Clip[] };
type Timeline = { id: string; name: string; external_refs: unknown[]; video_canvas: { width: number; height: number } | null; tracks: Track[]; markers: TimelineMarker[] };
...
const safeResponseError = async (response: Response) => {
  const payload = await response.json().catch(() => null) as { detail?: string | { code?: string; message?: string } } | null;
  const detail = payload?.detail;
  if (typeof detail === 'string') return detail;
  if (detail?.code === 'GUARD_CONFLICT') return 'This edit is protected by an active edit guard.';
...
function App() {
  ...
  const [moveDestinationFrame, setMoveDestinationFrame] = useState('0');
  ...
  const assetsById = new Map((project?.assets ?? []).map((item) => [item.id, item]));
  const clips = project?.timeline.tracks.flatMap((track) => track.clips.map((clip) => ({ ...clip, track_id: clip.track_id ?? track.id }))) ?? [];
  const selectedClip = clips.find((clip) => clip.id === selectedClipId) ?? null;
  const selectedTrack = selectedClip ? project?.timeline.tracks.find((track) => track.id === selectedClip.track_id) ?? null : null;
  const activeClipCount = clips.filter((clip) => frame >= clip.timeline_start_frame && frame < clip.timeline_start_frame + clipDuration(clip)).length;
  ...
  const previewAsset = playbackSample.kind === 'clip' ? assetsById.get(playbackSample.clip.asset_id) : project?.assets[0];
  ...
  useEffect(() => {
    setMoveDestinationFrame(selectedClip ? String(selectedClip.timeline_start_frame) : '0');
  }, [selectedClip?.id, selectedClip?.timeline_start_frame, project?.revision]);
  ...
  const refresh = (next: Project, selection: string | null = selectedClipId, markerSelection: string | null = selectedMarkerId) => {
    ...
    setProject(next);
    setSelectedClipId(selection && next.timeline.tracks.some((track) => track.clips.some((clip) => clip.id === selection)) ? selection : null);
    const selectedTrackId = selection ? next.timeline.tracks.find((track) => track.clips.some((clip) => clip.id === selection))?.id : null;
    const existingTargetId = targetTrackId && next.timeline.tracks.some((track) => track.id === targetTrackId) ? targetTrackId : null;
    setTargetTrackId(selectedTrackId ?? existingTargetId ?? next.timeline.tracks[0]?.id ?? null);
  ...
  const moveSelectedClip = () => {
    if (!selectedClip) { setError('Select a clip to move.'); return; }
    const destination = Number(moveDestinationFrame);
    if (!Number.isInteger(destination) || destination < 0) {
      setError('Move destination must be a non-negative integer frame.');
      return;
    }
    void mutate('move', { clip_id: selectedClip.id, timeline_start_frame: destination, target_track_id: targetTrackId }, selectedClip.id);
  };
    ...
    try {
      const response = await api<{ message: string; actions: ChatAction[] }>('/agent/chat', {
        ...
        body: JSON.stringify({ editor_session_id: editorSessionId, message, editor_context: {
          ...
          selected_clip_id: selectedClipId, playhead_frame: frame, visible_track_id: targetTrackId,
  ...
  const finishMove = () => {
    ...
    setMovePreview(null);
    if (gesture && preview) void mutate('move', { clip_id: gesture.clipId, timeline_start_frame: preview.timelineStartFrame, target_track_id: targetTrackId }, gesture.clipId);
  ...
  const dropClip = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    if (clipId) void mutate('move', { clip_id: clipId, timeline_start_frame: destination, target_track_id: targetTrackId }, clipId);
  };
  useEffect(() => {
  ...
  const addTrack = async () => {
    if (!project) return;
    try { refresh(await api<Project>(`/projects/${project.id}/tracks/add`, { method: 'POST', body: JSON.stringify({ name: `V${project.timeline.tracks.length + 1}`, expected_revision: project.revision }) })); }
    catch (exception) { setError(String(exception)); }
  };
  const renameTrack = async (track: Track) => {
    if (!project) return;
    const name = window.prompt('Track name', track.name)?.trim();
    if (!name || name === track.name) return;
    try { refresh(await api<Project>(`/projects/${project.id}/tracks/update`, { method: 'POST', body: JSON.stringify({ track_id: track.id, name, expected_revision: project.revision }) })); }
    catch (exception) { setError(String(exception)); }
  };
  const deleteTrack = async (track: Track) => {
    if (!project || track.clips.length) { setError('Only empty tracks can be deleted.'); return; }
    try { refresh(await api<Project>(`/projects/${project.id}/tracks/delete`, { method: 'POST', body: JSON.stringify({ track_id: track.id, expected_revision: project.revision }) })); }
    catch (exception) { setError(String(exception)); }
  };
  const reorderTrack = async (track: Track, position: number) => {
    if (!project) return;
    try { refresh(await api<Project>(`/projects/${project.id}/tracks/reorder`, { method: 'POST', body: JSON.stringify({ track_id: track.id, position, expected_revision: project.revision }) })); }
    catch (exception) { setError(String(exception)); }
  };

  return <main>
    {project && <div className="track-order-controls">{project.timeline.tracks.map((track, index) => <span key={track.id}>{track.name} <button disabled={index === 0} onClick={() => void reorderTrack(track, index - 1)}>Down</button><button disabled={index === project.timeline.tracks.length - 1} onClick={() => void reorderTrack(track, index + 1)}>Up</button></span>)}</div>}
    {project && <section className="stacked-timeline"><div className="track-controls"><button onClick={() => void addTrack()}>Add video track</button><label>Move target <select value={targetTrackId ?? ''} onChange={(event) => setTargetTrackId(event.target.value)}>{project.timeline.tracks.map((track) => <option key={track.id} value={track.id}>{track.name} · {track.kind}</option>)}</select></label></div>{project.timeline.tracks.slice().reverse().map((track) => <div key={track.id} className="video-track-lane"><div className="track-label">{track.name} · {track.kind} <button onClick={() => void renameTrack(track)}>Rename</button><button onClick={() => void deleteTrack(track)} disabled={track.clips.length > 0 || project.timeline.tracks.length === 1}>Delete</button></div><div className="track" onClick={(event) => { setTargetTrackId(track.id); const bounds = event.currentTarget.getBoundingClientRect(); seek(((event.clientX - bounds.left) / bounds.width) * displayTimelineDuration); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { setTargetTrackId(track.id); dropClip(event); }}>{track.clips.map((clip) => { const trim = trimPreview?.clipId === clip.id ? trimPreview : null; const move = movePreview?.clipId === clip.id ? movePreview : null; const sourceIn = trim ? trim.sourceInFrame : clip.source_in_frame; const sourceOut = trim ? trim.sourceOutFrame : clip.source_out_frame; const start = move ? move.timelineStartFrame : clip.timeline_start_frame; return <div key={clip.id} className={`clip ${clip.id === selectedClipId ? 'selected' : ''}`} onMouseDown={(event) => { setTargetTrackId(track.id); beginMove(event, clip); }} onClick={(event) => { event.stopPropagation(); setSelectedClipId(clip.id); setSelectedMarkerId(null); setError(''); }} style={{ left: `${start / displayTimelineDuration * 100}%`, width: `${(sourceOut - sourceIn) / displayTimelineDuration * 100}%` }}><span className="trim-handle trim-handle-in" onMouseDown={(event) => beginTrim(event, clip, 'in')} /><span className="clip-label">{assetsById.get(clip.asset_id)?.name ?? clip.asset_id}</span><span className="trim-handle trim-handle-out" onMouseDown={(event) => beginTrim(event, clip, 'out')} /></div>; })}<div className="playhead" style={{ left: `${frame / displayTimelineDuration * 100}%` }} /></div></div>)}</section>}
    <header><h1>TextSequence</h1><span>v0.5.0 · MCP-native NLE</span></header>
